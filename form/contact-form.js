...
for(let i=0;i<e$.length;i++){let e=e$[i];if(e.parentNode){e.parentNode.removeChild(e);}cssText+=e.textContent;}return cssText;}/**
   ...
   */ /* eslint-disable valid-jsdoc */ /**
                                        ...
                                        */const wrap=window['ShadyDOM']&&window['ShadyDOM']['noPatch']&&window['ShadyDOM']['wrap']?window['ShadyDOM']['wrap']:window['ShadyDOM']?n=>ShadyDOM['patch'](n):n=>n;var wrap$1={wrap:wrap};function isPath(path){return path.indexOf('.')>=0;}/**
...
let host;if(notified&&(host=inst.__dataHost)&&host._invalidateProperties){host._invalidateProperties();}}/**
   ...
   * @private
   * @suppress {invalidCasts}
   */function dispatchNotifyEvent(inst,eventName,value,path){let detail={value:value,queueProperty:true};if(path){detail.path=path;}wrap(/** @type {!HTMLElement} */inst).dispatchEvent(new CustomEvent(eventName,{detail}));}/**
...
return polymerElementBase._addTemplatePropertyEffect.call(this,templateInfo,prop,effect);}}return PolymerElement;});/**
     ...
     * @return {void}
     */const updateStyles=function(props){if(window.ShadyCSS){window.ShadyCSS.styleDocument(props);}};var elementMixin={version:version,ElementMixin:ElementMixin,updateStyles:updateStyles};class LiteralString{constructor(string){/** @type {string} */this.value=string.toString();}/**
   ...
   */const html=function html(strings,...values){const template=/** @type {!HTMLTemplateElement} */document.createElement('template');template.innerHTML=values.reduce((acc,v,idx)=>acc+htmlValue(v)+strings[idx+1],strings[0]);return template;};/**
    ...
    */const htmlLiteral=function(strings,...values){return new LiteralString(values.reduce((acc,v,idx)=>acc+literalValue(v)+strings[idx+1],strings[0]));};var htmlTag={html:html,htmlLiteral:htmlLiteral};const PolymerElement=ElementMixin(HTMLElement);var polymerElement={version:version,PolymerElement:PolymerElement,html:html};/**
   @license
   Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
   This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
   The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
...
let observer=new MutationObserver(()=>{if(wrap(this).querySelector('template')){observer.disconnect();this.__render();}else{throw new Error('dom-if requires a <template> child');}});observer.observe(this,{childList:true});return false;}this.__ctor=templatize(template,this,{// dom-if templatizer instances require `mutable: true`, as
...
this.__invalidProps=this.__invalidProps||Object.create(null);this.__invalidProps[root(prop)]=true;}}}});}if(!this.__instance){this.__instance=new this.__ctor();wrap(parentNode).insertBefore(this.__instance.root,this);}else{this.__syncHostProperties();let c$=this.__instance.children;if(c$&&c$.length){// Detect case where dom-if was re-attached in new position
let lastChild=wrap(this).previousSibling;if(lastChild!==c$[c$.length-1]){for(let i=0,n;i<c$.length&&(n=c$[i]);i++){wrap(parentNode).insertBefore(n,this);}}}}}return true;}__syncHostProperties(){let props=this.__invalidProps;if(props){for(let prop in props){this.__instance._setPendingProperty(prop,this.__dataHost[prop]);}this.__invalidProps=null;this.__instance._flushProperties();}}__teardownInstance(){if(this.__instance){let c$=this.__instance.children;if(c$&&c$.length){// use first child parent, for case when dom-if may have been detached
...
`;template.setAttribute('style','display: none;');document.head.appendChild(template.content);const template$1=html`
<custom-style>
  <style is="custom-style">
    html {
      /*
...
</custom-style>`;template$2.setAttribute('style','display: none;');document.head.appendChild(template$2.content);var style=document.createElement('style');style.textContent='[hidden] { display: none !important; }';document.head.appendChild(style);const IronFormElementBehavior={properties:{/**
...
if(value===undefined&&this.value!==undefined)this.invalid=!this._getValidity(this.value);else this.invalid=!this._getValidity(value);return!this.invalid;},/**
   ...
   * @return {boolean} True if `value` is valid.
   */_getValidity:function(value){if(this.hasValidator()){return this._validator.validate(value);}return true;}};var ironValidatableBehavior={get IronValidatableBehaviorMeta(){return IronValidatableBehaviorMeta;},IronValidatableBehavior:IronValidatableBehavior};const IronCheckedElementBehaviorImpl={properties:{/**
     ...
     * of the spacebar.
     */toggles:{type:Boolean,value:true,reflectToAttribute:true},/* Overriden from IronFormElementBehavior */value:{type:String,value:'on',observer:'_valueChanged'}},observers:['_requiredChanged(required)'],created:function(){// Used by `iron-form` to handle the case that an element with this behavior
...
if(this._controlStateChanged){this._controlStateChanged();}}};var ironControlState={IronControlState:IronControlState};var KEY_IDENTIFIER={'U+0008':'backspace','U+0009':'tab','U+001B':'esc','U+0020':'space','U+007F':'del'};/**
...
if(event.defaultPrevented){return;}}}},_triggerKeyHandler:function(keyCombo,handlerName,keyboardEvent){var detail=Object.create(keyCombo);detail.keyboardEvent=keyboardEvent;var event=new CustomEvent(keyCombo.event,{detail:detail,cancelable:true});this[handlerName].call(this,event);if(event.defaultPrevented){keyboardEvent.preventDefault();}}};var ironA11yKeysBehavior={IronA11yKeysBehavior:IronA11yKeysBehavior};const IronButtonStateImpl={properties:{/**
...
}}};/** @polymerBehavior */const IronButtonState=[IronA11yKeysBehavior,IronButtonStateImpl];var ironButtonState={IronButtonStateImpl:IronButtonStateImpl,IronButtonState:IronButtonState};var Utility={distance:function(x1,y1,x2,y2){var xDelta=x1-x2;var yDelta=y1-y2;return Math.sqrt(xDelta*xDelta+yDelta*yDelta);},now:window.performance&&window.performance.now?window.performance.now.bind(window.performance):Date.now};/**
    * @param {HTMLElement} element
    * @constructor
...
<div id="radioLabel"><slot></slot></div>`;template$3.setAttribute('strip-whitespace','');/**
                                                 ...
                                                 @demo demo/index.html
                                                 */Polymer({_template:template$3,is:'paper-radio-button',behaviors:[PaperCheckedElementBehavior],hostAttributes:{role:'radio','aria-checked':false,tabindex:0},properties:{/**
...
// centers align.
if(defaultInkSize%2!==size%2){defaultInkSize++;}this.updateStyles({'--paper-radio-button-ink-size':defaultInkSize+'px'});}});}});class IronSelection{/**
...
}else if(!this.hasAttribute('tabindex')){this.setAttribute('tabindex',this._previousTabIndex);}}};IronMenuBehaviorImpl._shiftTabPressed=false;/** @polymerBehavior */const IronMenuBehavior=[IronMultiSelectableBehavior,IronA11yKeysBehavior,IronMenuBehaviorImpl];var ironMenuBehavior={IronMenuBehaviorImpl:IronMenuBehaviorImpl,IronMenuBehavior:IronMenuBehavior};const IronMenubarBehaviorImpl={hostAttributes:{'role':'menubar'},/**
   * @type {!Object}
...
this._focusWithKeyboardEvent(event);}};/** @polymerBehavior */const IronMenubarBehavior=[IronMenuBehavior,IronMenubarBehaviorImpl];var ironMenubarBehavior={IronMenubarBehaviorImpl:IronMenubarBehaviorImpl,IronMenubarBehavior:IronMenubarBehavior};Polymer({_template:html`
    <style>
      :host {
        display: inline-block;
      }
...
</custom-style>`;template$4.setAttribute('style','display: none;');document.head.appendChild(template$4.content);const PaperInputAddonBehavior={attached:function(){this.fire('addon-attached');},/**
...
`,is:'paper-input-char-counter',behaviors:[PaperInputAddonBehavior],properties:{_charCounterStr:{type:String,value:'0'}},/**
   ...
   *     value: The input value.
   *     invalid: True if the input value is invalid.
   */update:function(state){if(!state.inputElement){return;}state.value=state.value||'';var counter=state.value.toString().length.toString();if(state.inputElement.hasAttribute('maxlength')){counter+='/'+state.inputElement.getAttribute('maxlength');}this._charCounterStr=counter;}});const template$5=html`
<custom-style>
  <style is="custom-style">
    html {
...
this._focusableElement.focus();}}}};/** @polymerBehavior */const PaperInputBehavior=[IronControlState,IronA11yKeysBehavior,PaperInputBehaviorImpl];var paperInputBehavior={PaperInputHelper:PaperInputHelper,PaperInputBehaviorImpl:PaperInputBehaviorImpl,PaperInputBehavior:PaperInputBehavior};Polymer({is:'paper-input',_template:html`
    <style>
      :host {
        display: block;
      }
...
if(!!this.inputElement.bindValue){this.$.container._handleValueAndAutoValidate(this.inputElement);}}});const template$6=html`
<custom-style>
  <style is="custom-style">
    html {
...
</dom-module>`;template$7.setAttribute('style','display: none;');document.head.appendChild(template$7.content);const PaperButtonBehaviorImpl={properties:{/**
...
if(this.hasRipple()&&this.getRipple().ripples.length<1){this._ripple.uiDownAction();}},/**
   ...
   *
   * @param {!KeyboardEvent} event .
   */_spaceKeyUpHandler:function(event){IronButtonStateImpl._spaceKeyUpHandler.call(this,event);if(this.hasRipple()){this._ripple.uiUpAction();}}};/** @polymerBehavior */const PaperButtonBehavior=[IronButtonState,IronControlState,PaperRippleBehavior,PaperButtonBehaviorImpl];var paperButtonBehavior={PaperButtonBehaviorImpl:PaperButtonBehaviorImpl,PaperButtonBehavior:PaperButtonBehavior};const template$8=html`
  ...
  <slot></slot>`;template$8.setAttribute('strip-whitespace','');/**
                                                 Material design:
                                                 ...
                                                 `--paper-button-raised-keyboard-focus` | Mixin applied to a raised button after it's been focused using the keyboard | `{}`
                                                                                               @demo demo/index.html
                                                 */Polymer({_template:template$8,is:'paper-button',behaviors:[PaperButtonBehavior],properties:{/**
     * If true, the button should be styled with a shadow.
     */raised:{type:Boolean,reflectToAttribute:true,value:false,observer:'_calculateElevation'}},_calculateElevation:function(){if(!this.raised){this._setElevation(0);}else{PaperButtonBehaviorImpl._calculateElevation.apply(this);}}/**
    Fired when the animation finishes.
    This is useful if you want to wait until
    the ripple animation finishes to perform some action.
     @event transitionend
...
`,is:'paper-card',properties:{/**
   ...
   */_isHidden:function(image){return image?'false':'true';},_headingChanged:function(heading){var currentHeading=this.getAttribute('heading'),currentLabel=this.getAttribute('aria-label');if(typeof currentLabel!=='string'||currentLabel===currentHeading){this.setAttribute('aria-label',heading);}},_computeHeadingClass:function(image){return image?' over-image':'';},_computeAnimated:function(animatedShadow){return animatedShadow;}});/**
    @license
...
</dom-module>`;document.head.appendChild($_documentContainer.content);const PaperSpinnerBehavior={properties:{/**
...
if(alt==='loading'){this.alt=this.getAttribute('aria-label')||alt;}else{this.__setAriaHidden(alt==='');this.setAttribute('aria-label',alt);}},__setAriaHidden:function(hidden){var attr='aria-hidden';if(hidden){this.setAttribute(attr,'true');}else{this.removeAttribute(attr);}},__reset:function(){this.active=false;this.__coolingDown=false;}};var paperSpinnerBehavior={PaperSpinnerBehavior:PaperSpinnerBehavior};const template$9=html`
...
`;template$9.setAttribute('strip-whitespace','');/**
                                                 ...
                                                 @hero hero.svg
                                                 @demo demo/index.html
                                                 */Polymer({_template:template$9,is:'paper-spinner',behaviors:[PaperSpinnerBehavior]});Polymer({is:'iron-request',hostAttributes:{hidden:true},properties:{/**
...
this.fire('iron-ajax-error',{request:request,error:error},{bubbles:this.bubbles,composed:true});this.fire('error',{request:request,error:error},{bubbles:this.bubbles,composed:true});},_discardRequest:function(request){var requestIndex=this.activeRequests.indexOf(request);if(requestIndex>-1){this.splice('activeRequests',requestIndex,1);}},_requestOptionsChanged:function(){this.debounce('generate-request',function(){if(this.url==null){return;}if(this.auto){this.generateRequest();}},this.debounceDuration);}});class ContactForm extends PolymerElement{static get template(){return html`
      ...
      <p hidden="[[!showMessage]]">Замовлення успішно завершено. Наш менеджер зателефонує до Вас, лише якщо буде необхідність уточнити деталі замовлення. Після здійснення відправки Вам буде надіслано номер накладної. Дякуємо за замовлення! <a href="/">Повернутись</a></p>
      </paper-card>
    ...
    `;}static get properties(){return{loading:{type:Boolean,value:false},hideForm:{type:Boolean,value:false},showMessage:{type:Boolean,value:false}};}_validate(){let validInputs=0;const inputs=this.shadowRoot.querySelectorAll('paper-input');inputs.forEach(input=>{input.validate();if(input.inputElement.querySelector('input').validity.valid){validInputs++;}});const isValid=inputs.length===validInputs;const ajax=this.shadowRoot.querySelector('iron-ajax');let ajaxParams={};if(isValid){inputs.forEach(input=>{ajaxParams[input.id]=input.value;});ajaxParams.paymentType=this.$.paymentType.selected;ajax.params=ajaxParams;this.loading=true;ajax.generateRequest();}}_handleResponse(){this.hideForm=true;const isOnlinePayment=this.$.paymentType.selected==='Online';if(isOnlinePayment){window.location='https://secure.wayforpay.com/button/b0920bb075de9';}else{this.showMessage=true;}this.loading=false;}_handleError(){this.loading=false;}}window.customElements.define('contact-form',ContactForm);export{applyShim as $applyShim$1,ApplyShim as $applyShimDefault,applyShimUtils as $applyShimUtils,arraySelector as $arraySelector,arraySplice as $arraySplice,async as $async,caseMap$1 as $caseMap,_class as $class,commonRegex as $commonRegex,commonUtils as $commonUtils,cssParse as $cssParse,customStyle as $customStyle,customStyleInterface as $customStyleInterface$1,CustomStyleInterface as $customStyleInterfaceDefault,debounce as $debounce,dirMixin as $dirMixin,documentWait$1 as $documentWait,documentWait as $documentWaitDefault,domBind as $domBind,domIf as $domIf,domModule as $domModule,domRepeat as $domRepeat,elementMixin as $elementMixin,flattenedNodesObserver as $flattenedNodesObserver,flush$2 as $flush,gestureEventListeners as $gestureEventListeners,gestures$1 as $gestures,hideTemplateControls as $hideTemplateControls,htmlTag as $htmlTag,ironA11yAnnouncer as $ironA11yAnnouncer,ironA11yKeysBehavior as $ironA11yKeysBehavior,ironButtonState as $ironButtonState,ironCheckedElementBehavior as $ironCheckedElementBehavior,ironControlState as $ironControlState,ironFormElementBehavior as $ironFormElementBehavior,ironMenuBehavior as $ironMenuBehavior,ironMenubarBehavior as $ironMenubarBehavior,ironMeta as $ironMeta,ironMultiSelectable as $ironMultiSelectable,ironSelectable as $ironSelectable,ironSelection as $ironSelection,ironValidatableBehavior as $ironValidatableBehavior,legacyElementMixin as $legacyElementMixin,mixin as $mixin,mutableData as $mutableData,mutableDataBehavior as $mutableDataBehavior,paperButtonBehavior as $paperButtonBehavior,paperCheckedElementBehavior as $paperCheckedElementBehavior,paperInkyFocusBehavior as $paperInkyFocusBehavior,paperInputAddonBehavior as $paperInputAddonBehavior,paperInputBehavior as $paperInputBehavior,paperRippleBehavior as $paperRippleBehavior,paperSpinnerBehavior as $paperSpinnerBehavior,path as $path,polymer_dom as $polymerDom,polymerElement as $polymerElement,polymerFn as $polymerFn,polymerLegacy as $polymerLegacy,propertiesChanged as $propertiesChanged,propertiesMixin as $propertiesMixin,propertyAccessors as $propertyAccessors,propertyEffects as $propertyEffects,renderStatus as $renderStatus,resolveUrl$1 as $resolveUrl,scopeSubtree$1 as $scopeSubtree,settings as $settings,styleGather as $styleGather,styleSettings as $styleSettings,styleUtil as $styleUtil,telemetry as $telemetry,templateMap$1 as $templateMap,templateMap as $templateMapDefault,templateStamp as $templateStamp,templatize$1 as $templatize,templatizerBehavior as $templatizerBehavior,unscopedStyleHandler as $unscopedStyleHandler,wrap$1 as $wrap,ANIMATION_MATCH,ArraySelector,ArraySelectorMixin,BRACKETED,Base,Class,CustomStyle,CustomStyleInterfaceInterface,CustomStyleProvider,Debouncer,DirMixin,DomApi,DomBind,DomIf,DomModule,DomRepeat,ElementMixin,EventApi,FlattenedNodesObserver,GestureEventListeners,HOST_PREFIX,HOST_SUFFIX,IS_VAR,IronA11yAnnouncer,IronA11yKeysBehavior,IronButtonState,IronButtonStateImpl,IronCheckedElementBehavior,IronCheckedElementBehaviorImpl,IronControlState,IronFormElementBehavior,IronMenuBehavior,IronMenuBehaviorImpl,IronMenubarBehavior,IronMenubarBehaviorImpl,IronMeta,IronMultiSelectableBehavior,IronMultiSelectableBehaviorImpl,IronSelectableBehavior,IronSelection,IronValidatableBehavior,IronValidatableBehaviorMeta,LegacyElementMixin,MEDIA_MATCH,MIXIN_MATCH,MutableData,MutableDataBehavior,OptionalMutableData,OptionalMutableDataBehavior,PaperButtonBehavior,PaperButtonBehaviorImpl,PaperCheckedElementBehavior,PaperCheckedElementBehaviorImpl,PaperInkyFocusBehavior,PaperInkyFocusBehaviorImpl,PaperInputAddonBehavior,PaperInputBehavior,PaperInputBehaviorImpl,PaperInputHelper,PaperRippleBehavior,PaperSpinnerBehavior,Polymer,Polymer as Polymer$1,PolymerElement,PropertiesChanged,PropertiesMixin,PropertyAccessors,PropertyEffects,StyleNode,TemplateInstanceBase,TemplateStamp,Templatizer,VAR_ASSIGN,VAR_CONSUMED,add,enqueueDebouncer as addDebouncer,addListener,afterNextRender,allowTemplateFromDomModule,animationFrame,applyCss,applyStyle,applyStylePlaceHolder,beforeNextRender,calculateSplices,camelToDashCase,cancelSyntheticClickEvents,createScopeStyle,cssBuild,cssFromModule,cssFromModuleImports,cssFromModules,cssFromTemplate,dashToCamelCase,dedupingMixin,deepTargetFind,detectMixin,disableRuntime,dom,dumpRegistrations,elementHasBuiltCss,elementsAreInvalid,enqueueDebouncer,enqueueDebouncer as enqueueDebouncer$1,findMatchingParen,findOriginalTarget,flush$1 as flush,flush$1,flush as flush$2,flushDebouncers,forEachRule,gatherStyleText,gestures,get,getBuildComment,getComputedStyleValue,getCssBuild,getIsExtends,hideElementsGlobally,html,html as html$1,html as html$2,htmlLiteral,idlePeriod,incrementInstanceCount,instanceCount,invalidate,invalidateTemplate,isAncestor,isDeep,isDescendant,isKeyframesSelector,isOptimalCssBuild,isPath,isTargetedBuild,isUnscopedStyle,isValid,isValidating,legacyOptimizations,matches,matchesSelector,microTask,mixinBehaviors,modelForElement,nativeCssVariables,nativeShadow,normalize,parse,passiveTouchGestures,pathFromUrl,prevent,processUnscopedStyle,processVariableAndFallback,recognizers,register$1 as register,register as register$1,registrations,remove,removeCustomPropAssignment,removeListener,resetMouseCanceller,resolveCss,resolveUrl,root,rootPath,rulesForStyle,sanitizeDOMValue,scopeSubtree,scopingAttribute,set,setAllowTemplateFromDomModule,setCancelSyntheticClickEvents,setElementClassRaw,setLegacyOptimizations,setPassiveTouchGestures,setRootPath,setSanitizeDOMValue,setStrictTemplatePolicy,setSyncInitialRender,setTouchAction,split,splitSelectorList,startValidating,startValidatingTemplate,strictTemplatePolicy,stringify,stylesFromModule,stylesFromModuleImports,stylesFromModules,stylesFromTemplate,syncInitialRender,templateIsValid,templateIsValidating,templatize,timeOut,toCssText,translate,types,updateNativeProperties,updateStyles,useNativeCSSProperties,useNativeCustomElements,useShadow,version,version as version$1,wrap,wrap$2 as wrap$1};